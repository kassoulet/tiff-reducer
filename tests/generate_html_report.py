...
"""
            ...
            if "psnr" in result:
                psnr_str = (
                    "∞" if result["psnr"] == float("inf") else f"{result['psnr']:.2f}"
                )
                html += f"""
            <div class="metrics">
                <div class="metric">
...
def run_tests(
    ...
    for image_path in test_images:
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            # Compress
            success, comp_size, reduction = compress_file(
                image_path, comp_path, binary_path, format, level
            )
            ...
            create_thumbnail(
                image_path, os.path.join(output_dir, "thumbnails", thumb_orig)
            )
            create_thumbnail(
                comp_path, os.path.join(output_dir, "thumbnails", thumb_comp)
            )
            create_diff_image(
                image_path,
                comp_path,
                os.path.join(output_dir, "thumbnails", thumb_diff),
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Generate HTML Visual Test Report for tiff-reducer"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        "--limit", "-n", type=int, default=20, help="Limit number of test images"
    )
    ...
    # Run tests
    run_tests(
        [str(p) for p in test_images], args.binary, args.output, args.format, args.level
    )
