...
def get_tiffinfo(filepath):
    ...
    try:
        result = subprocess.run(
            ["tiffinfo", str(filepath)], capture_output=True, text=True, timeout=30
        )
...
def main():
    ...
    for i, image_path in enumerate(images):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            success, error = test_compression(
                image_path, output_path, binary_path, args.format, args.level
            )
...
def generate_report(results, all_images, output_dir):
    ...
    report.append("|--------------|-------|")
    report.append(
        f"| TIFFWriteDirectorySec crash | {len(results['failed_directory'])} |"
    )
    ...
    report.append("")
    report.append(
        f"**{working} images** successfully compressed with thumbnails below:"
    )
    report.append("")
