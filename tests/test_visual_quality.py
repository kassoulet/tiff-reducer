...
def compare_tiffs(original_path: str, compressed_path: str) -> dict:
    ...
    # Check dimensions
    if (
        orig_ds.RasterXSize != comp_ds.RasterXSize
        or orig_ds.RasterYSize != comp_ds.RasterYSize
    ):
...
def main():
    ...
    try:
        ...
        if results["dimensions_match"]:
            print(f"\nPer-band metrics:")
            for i, (psnr, ssim) in enumerate(
                zip(results["psnr_per_band"], results["ssim_per_band"])
            ):
